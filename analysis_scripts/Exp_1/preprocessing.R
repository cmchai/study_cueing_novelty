...
### Step 1. load big dataframe and all the necessary packages ###
setwd("D:/Ghent_Braem/Experiment/1st_research_PRO/data/raw_data")
library(feather)
library(tidyverse)

bigdf <- read_feather("bigdf_full.feather")
# save(bigdf, file = "D:/Ghent_Braem/Experiment/1st_research_PRO/data/raw_data/raw_data_exp1.RData")
...
### Add another column to generate a unique trial number combining subject, block, and trial information ###

bigdf <- mutate(bigdf, trialID = str_c(subject, block, trial_num, sep = "_"))

index_target <- which((bigdf$block_type %in% c("pracMix", "testCue", "testNocue")) & bigdf$trial_part == "target")
index_taskcue <- which((bigdf$block_type %in% c("pracMix", "testCue", "testNocue")) & bigdf$trial_part == "taskcue")
bigdf$trialID[index_taskcue] <- bigdf$trialID[index_target]
...
acc_threshold <- 0.8 # we use 0.7 across all experiments, and 0.8 for the pre-registration
...
lowacc_subject <- bigdf %>%
  ...
  mutate(exclude = (block_dich == "prac" & accRate < acc_threshold) | (block_dich == "test" & accRate < 0.55)) %>%
  filter(exclude == TRUE) %>%
  ungroup()
...
# for the task cue processing RT
taskcue_outlier_subject <- bigdf_ex_lowacc_sub %>%
  filter(block_dich == "test" & trial_part == "taskcue") %>%
  group_by(subject) %>%
  summarise(
    count = n(),
    meanRT = mean(rt, na.rm = TRUE),
  ...
  filter(exclude_RT == TRUE) %>%
  ungroup()


# exclude outliers

outlier_subject <- unique(c(unique(target_outlier_subject$subject), 
                            unique(taskcue_outlier_subject$subject)))

bigdf_clean_subject <- filter(bigdf_ex_lowacc_sub, ! (subject %in% outlier_subject))
write_feather(bigdf_clean_subject, "bigdf_cleansubject.feather")

(nrow(bigdf_ex_lowacc_sub) - nrow(bigdf_clean_subject))/1208 # should equals to the number of subject ourliers


# add a colume specifying if the trial is either repeat or switch, only for the testing section

trial_label_switch <- bigdf_clean_subject %>%
  filter(block_dich == "test", trial_part == "target") %>%
...
short_RT <- bigdf_clean_subject %>%
  filter(block_dich == "test" & (trial_part %in% c("target", "taskcue")) & rt <= 200)
...
### Step 8. exclude the trials with > 5s on task cue processing ###

long_taskcue_rt_trials <- bigdf_clean_subject %>%
  filter(block_dich == "test", trial_part == "taskcue", rt > 5000)

long_taskcue_trialID <- unique(long_taskcue_rt_trials$trialID)
...
exclude_trials <- unique(c(short_trialID,
                          error_trialID,
                          post_error_trialID,
                          long_taskcue_trialID))

bigdf_ex_short_error_post <- filter(bigdf_clean_subject, !(trialID %in% exclude_trials))

(nrow(bigdf_clean_subject) - nrow(bigdf_ex_short_error_post))/2 - 67 - 1552 - 1298 # the number of long taskcue RT trials
...
# During the task cue processing part
outlier_taskcue_trials <- bigdf_ex_short_error_post %>%
  group_by(subject) %>%
  filter(block_dich == "test" & trial_part == "taskcue") %>%
  mutate(mean_RT_subject = mean(rt, na.rm = TRUE),
         median_RT_subject = median(rt, na.rm = TRUE),
...
outlier_taskcue_trialID <- unique(outlier_taskcue_trials$trialID)


exclude_trial_step2 <- unique(c(outlier_target_trialID,
                                outlier_taskcue_trialID))

bigdf_clean <- filter(bigdf_ex_short_error_post, !(trialID %in% exclude_trial_step2))
...
write_feather(bigdf_clean, "bigdf_clean_highaccthre.feather")
...
# recalculating the outliers During the task cue processing part
outlier_taskcue_error_trials <- bigdf_ex_short_post %>%
  group_by(subject) %>%
  filter(block_dich == "test" & trial_part == "taskcue") %>%
  mutate(mean_RT_subject = mean(rt, na.rm = TRUE),
         median_RT_subject = median(rt, na.rm = TRUE),
...
outlier_taskcue_error_trialID <- unique(outlier_taskcue_error_trials$trialID)

exclude_trial_error_step2 <- unique(c(outlier_target_error_trialID,
                                      outlier_taskcue_error_trialID))

bigdf_clean_error <- filter(bigdf_ex_short_post, !(trialID %in% exclude_trial_error_step2))


write_feather(bigdf_clean_error, "bigdf_clean_error_highaccthre.feather")
