...
load(file = paste0(here(), "/results/bigdf_clean_subject.Rdata"))
...
## add another column specifying the switch type of the previous trial ----
back2_switch_df <- bigdf_clean_subject %>%
  convert_as_factor(switch_type) %>%
  mutate(switch_or_repeat = if_else(str_detect(switch_type, "switch"), "switch", "repeat"),
         pre_switch_or_repeat = lag(switch_or_repeat)) %>%
  select(trial_id, pre_switch_or_repeat)

save(back2_switch_df, file = paste0(here(), "/results/2back_switch.Rdata"))
