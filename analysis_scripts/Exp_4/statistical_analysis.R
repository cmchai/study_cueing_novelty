...
######## for PRO paradigm Exp.3 Replication study ###########
library(here)
library(tidyverse)
library(ez)
library(rstatix)
...
# load(file = paste0(here(), "/results/bigdf_clean_highaccthre.Rdata")) # the dataset after the high acc threshold exclusion criterion
...
rt_target_help_anova$ANOVA

# visualize and explore the main effect of cue validity
...
emm <- emmeans(rt_target_help_anova$aov, pairwise ~ cue_validity)
emm
...
# load(file = paste0(here(), "/results/bigdf_clean_error_highaccthre.Rdata"))
...
targetACC_descrip_cue <- test_target_error_df %>%
  group_by(subject, cue_validity) %>%
  summarise(count = n(),
            accuracy_rate = sum(accuracy)/length(accuracy)) %>%
  ungroup() %>%
...
acc_rate_subject2 <- test_target_error_df %>%
  group_by(subject, test_sequence, cue_validity) %>%
  summarise(count = n(),
            accuracy_rate = sum(accuracy)/length(accuracy))

(p_target_rt_inter <- ggplot(acc_rate_subject2, aes(x = test_sequence, y = accuracy_rate, fill = cue_validity)) +
    geom_violin(position = dodge, size = 1) +
    scale_fill_manual(values = alpha(c("#4E84C4", "#FC4E07"), .5)) +
    geom_boxplot(width = 0.3, position = dodge, size = 1)  +
    stat_summary(fun = mean, geom="point", shape=20, size=4, color="black", position = dodge) +
    ggtitle("interaction between test sequence and cue validity in acc rate") +
    theme_bw())
...
# calculate and analyze the switch cost for each condition ----
...
# calculate the analyze acc rate Switch Cost ----
...
save(switch_type_acc_bysubject_2, file = paste0(here(), "/results/data_for_ANOVA_exp4.Rdata")) # save for across exp ananlysis with exp3
...
(p_target_rt2 | p_acc_rate2) / p_switch_type_acc2 + 
  plot_annotation(title = 'The behavioral results of Experiment 4',
                  tag_levels = 'A')
